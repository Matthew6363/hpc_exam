...
inline int inject_energy ( const int     periodic,
			   ...
			   const double  energy,
			   const int     mysize[2],
                           double *plane ) /*mysize 1D array for x and y dimension for a 2D grid*/
{
   #define IDX( i, j ) ( (j)*(mysize[_x_]+2) + (i) ) /* macro for index of 1D matrix (+2 for ghost cells)*/
 ...
 */
{
   ...
   #define IDX( i, j ) ( (j)*fxsize + (i) )
    ...
    if ( periodic )
         ...
         * 
         * Important when grid sections managed by different processes need to exchange edge data to maintain the correct simulation behavior. 
         * This ensures that the periodicity is preserved even when the grid is divided among multiple processors.
         */
        {
            for ( int i = 1; i <= xsize; i++ )
                {
                    new[ i ] = new[ IDX(i, ysize) ];
                    new[ IDX(i, ysize+1) ] = new[ i ]; // IDX(i,1)
                }
            for ( int j = 1; j <= ysize; j++ )
                {
